...
namespace App1
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void gameOverText_SelectionChanged(object sender, RoutedEventArgs e) 
        {
        ...
        private void playArena_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if(humanCaptured)
            {
                Point pointerPosition = e.GetCurrentPoint(null).Position;
                Point relativePosition = grid.TransformToVisual(playArena).TransformPoint(pointerPosition);
                if ((Math.Abs(relativePosition.X - Canvas.GetLeft(human)) > human.ActualWidth * 3)
                    || (Math.Abs(relativePosition.Y - Canvas.GetTop(human)) > human.ActualHeight))
                {
                    humanCaptured = false;
                    human.IsHitTestVisible = true;
                }
        ...
        private void playArena_PointerExited(object sender, PointerRoutedEventArgs e)
        {
            if (humanCaptured)
                EndTheGame();
        }
